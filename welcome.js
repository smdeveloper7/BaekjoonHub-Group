...
const getCustomDirPath = () => {
  return $('#custom_root_dir_path').val().trim();
};
...
// Root Directory option 변경시
$('#root_dir_option').on('change', function () {
  const valueSelected = this.value;
  if (valueSelected === 'custom') {
    $('#custom_root_dir_path_div').show(); // Show the custom directory input field
  } else {
    $('#custom_root_dir_path_div').hide(); // Hide the custom directory input field
  }
});
...
$('#hook_button').on('click', () => {
  ...
  if (!option()) {
    ...
    $('#error').show();
    // 추가가
  }else if ($('#root_dir_option').val() === 'custom' && !getCustomDirPath()) {
      $('#error').text('No directory path added - Enter the path of your directory!');
      $('#custom_root_dir_path').focus();
      $('#error').show();
  } else if ($('#root_dir_option').val() === 'custom' && !isValidDirPath(getCustomDirPath())) {
      let errorMessage = 'Invalid directory path - Make sure the directory path is valid!';
      const dirPath = getCustomDirPath();
    ...
    const root_dir_option = $('#root_dir_option').val();
    chrome.storage.local.set({ BaekjoonHub_PushDirOption: root_dir_option }, () => {
      console.log(`Set PushDirOption by ${root_dir_option}`);
    });
    if (root_dir_option === 'custom') {
      const custom_root_dir_path = getCustomDirPath();
      chrome.storage.local.set({ BaekjoonHub_PushDirPath: custom_root_dir_path }, () => {
        console.log(`Set PushDirPath by ${custom_root_dir_path}`);
      });
    }
